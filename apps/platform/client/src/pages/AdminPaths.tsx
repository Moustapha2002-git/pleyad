...
export default function AdminPaths() {
  ...
  const createPath = trpc.paths.create.useMutation({
    onSuccess: (res) => {
      toast.success("Path created — now add its skills");
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="p-6">
          ...
          >
            ...
            <Button type="submit" disabled={!title.trim() || createPath.isPending}>
              {createPath.isPending ? "Creating…" : "Create & add skills"}
            </Button>
          </form>
        </Card>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat icon={RouteIcon} value={data?.stats.total ?? "…"} label="Paths" />
        <Stat
          icon={Users}
      ...
      {dir.isLoading ? (
        ...
        <EmptyState
          ...
          description="Create the first learning path with the button above."
        />
      ) : (
            ...
            <Card key={p.id} className="flex flex-col overflow-hidden">
              ...
              <div className="flex flex-1 flex-col gap-2 p-4">
                ...
                <div className="mt-auto flex items-center gap-2 border-t border-gray-100 pt-3">
                  ...
                  >
                    <Pencil className="h-3.5 w-3.5" /> Manage & assign
                  </button>
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          `Delete "${p.title}"? ${p.enrolledCount} enrollment${p.enrolledCount === 1 ? "" : "s"} will be removed. Learners' personal progress records and the courses themselves are kept.`,
                        )
                      )
